...
const items = [
  { id: 'recents', label: 'Gần đây' },
  { id: 'home', label: 'Trang chủ' },
  { id: 'applications', label: 'Ứng dụng' },
  { id: 'desktop', label: 'Màn hình' },
...
const displayFormSchema = z.object({
  items: z.array(z.string()).refine((value) => value.some((item) => item), {
    message: 'Bạn phải chọn ít nhất 1 mục.'
  })
})
    ...
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit((data) => {
          showSubmittedData(data, 'Bạn đã cập nhật hiển thị:')
          ...
          render={() => (
            <FormItem>
              <div className="mb-4">
                ...
                <FormDescription>Chọn các mục bạn muốn hiển thị ở thanh bên.</FormDescription>
              </div>
              {items.map((item) => (
                <FormField
                  key={item.id}
